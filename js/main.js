...
function loadProfile() {
  ...
  if (localStorage.getItem("posts")) {
    ...
    obj.reverse().forEach((element) => {
      // console.log("element: ", element);
      let post_div = `<div class="post_div">
        <img id="${element['filename']}" src="${element['image']}" class="post_image""/>
...
$(document).ready(function(){
  ...
  $(".post_image").click(function(){
    let dataFind = JSON.parse(localStorage.getItem("posts"));

    let like = $(this).siblings('.post_action').children('.output');
    console.log(this);
    ...
    if (true) {
      ...
      dataFind.forEach((element) =>{
        ...
        if(element.filename==this.id){
          ...
          element.likes=++count;
          like.html(count)
          localStorage.setItem("posts", JSON.stringify(dataFind));
        }
      })
    }
    else
